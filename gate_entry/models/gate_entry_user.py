...
class UserLogin(models.TransientModel):
    ...
    def login(self):
        u_name = self.env['gate.user.registration'].search([('username', '=', self.username)]) #This line searches if the given user is registered or not, if yes variable is assigned with the record having that name
...
class LoginDisplay(models.TransientModel):
    ...
    warehouse_id = fields.Many2one('stock.warehouse',string='Warehouse')
    #This method opens the inward gate in form view with the username and warehouse captured on opening, when inward button is clicked
    def Inward(self): 
        return {
            'name': ('gate_entry_form_view'),
            'view_mode': 'form',
    ...
    #This method opens the outward gate in form view with the username and warehouse captured on opening, when outward button is clicked
    def Outward(self):
        return {
            'name': ('gate_entry_form_view'),
            'view_mode': 'form',
