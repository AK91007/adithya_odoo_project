...
class GateEntryLine(models.Model):
    ...
    purchase_order_inward_ids = fields.Many2one("purchase.order", string="Purchase Order") #This field determines the PO to which the gate entry belongs (useful) for backorders
    ...
    #This onchange method filters out the relevant outgoing transactions for the user to choose
...
class GateEntryName(models.Model):
    ...
    #This method is used to raise user error in case of blank sequence fields
...
#This class is defined to capture the warehouse in the other information tab of PO 
...
class StockPicking(models.Model):
    ...
    gate_entry_id = fields.Many2one('gate.entry',string="Security Check Reference",copy=False)
    # gate_entry_purchase_outward_id = fields.Many2one('gate.entry',copy=False)
    purchase_return = fields.Boolean(store=True,compute='compute_purchase_return') #This boolean identifies if a picking is return or not
    purchase_boolean = fields.Boolean(string='Purchase Boolean',store=True,compute='compute_purchase_boolean') #This boolean identifies if picking belongs to a PO or not
